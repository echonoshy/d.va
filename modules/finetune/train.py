...
def train_autoencoder(
    ...
    for _epoch in range(1 if validate else epochs):
        ...
        if validate:
            ...
            header: str = "{blue_light}{0}: {1}{reset}".format("Epoch", output_iter(_epoch, epochs), **ansi)
...
def train_gpt(
    ...
    for speaker in dataset.speakers:
        if speaker not in speaker_embeds:
            speaker_embeds[speaker] = chat.speaker._sample_random().to(device=chat.device)
    ...
    for _epoch in range(1 if validate else epochs):
        ...
        for batch in iterator:
            ...
            text_input_ids: torch.Tensor = batch["text_input_id"]  # (batch_size, text_len)
